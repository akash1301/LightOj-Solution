	...
	while(t--)
    {
        ...
        loop(i,n)
        {
            for(int j=0;j<=tot;j++)
            {
                ...
                int dif=abs(j-data[i]);
                dp[cur][j]=max(dp[cur][j],dp[prev][j]);
                if(dif<=half)
                    dp[cur][dif]=max(dp[cur][dif],dp[prev][j]+data[i]);
                if(j+data[i]<=half)
                    dp[cur][j+data[i]]=max(dp[cur][j+data[i]],dp[prev][j]+data[i]);
            }
            tot=min(tot+data[i],half);
            cur^=1;
            prev^=1;
        }
        int ans=dp[prev][0]/2;
