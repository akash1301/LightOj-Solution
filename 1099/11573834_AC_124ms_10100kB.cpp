...
int n,r,vis[mx],bac[mx];
vector< pair<int,ll> >g[mx];
ll dis[mx][5],mincost[mx];
...
void dijkstra()
{
    ...
    for(int i=1;i<=n;i++)
    {
        loop(j,g[i].size())
        {
            mincost[i]=min(mincost[i],2*g[i][j].sc);
        }
    }
    while(!pq.empty())
    {
        ...
        loop(i,g[u].size())
        {
            ...
            else if(dis[v][0]<new_cost && dis[v][1]>new_cost)
            {
                //dis[v][1]=new_cost;
                pq.push(node(v,new_cost));
            }
